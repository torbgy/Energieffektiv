...
#include <fcntl.h>
#include <signal.h>
#define SIG_NUM 44 // Signal number
...
{   
    ...
    // Memory Map
    uint16_t* display = (uint16_t*)mmap(0,320*240*2, PROT_READ | PROT_WRITE, MAP_SHARED, fd_fb, 0);
    // Signals
    ...
    while(1){
        ...
        while(reset == 0){        
            ...
            if ( display[next.y*320+next.x] == 0xffff){
                ...
                if ((snakeTail + snakeLength - 1) < 400){
                    snake[snakeTail + snakeLength -1 ] = next;
                }else{
                    snake[snakeTail + snakeLength -1 - 400] = next;
                }
                ...
                // Delete tail visual
                rect.dx = snake[snakeTail].x;
                rect.dy = snake[snakeTail].y;
            ...
            ioctl(fd_fb, 0x4680, &rect);
            /*
            printf("snake: snakeLength: %d snakeTail: %d\n",snakeLength,snakeTail );
            for(x=snakeTail;x<snakeTail + snakeLength;x++){
                if ((x) < 400){
                    printf("[%d , %d]\n",snake[x].x,snake[x].y );
                }else{
                   printf("[%d , %d]\n",snake[x-400].x,snake[x-400].y );
                }
            }
            */
        ...
        // because we did not take the time to make text for the display.
        printf("Your score: %d\n",snakeLength);
